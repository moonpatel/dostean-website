    ...
    return (
        <div className='grid grid-cols-1 gap-x-20 md:grid-cols-2 w-4/5 mx-auto gap-3'>
            {dishes.map(({ name, desc, price, lprice, sprice }) => {
                return <div className='flex justify-between p-2 bg-yellow-400 m-2 w-full h-24 md:h-20 group shadow-lg col-span-2 md:col-span-1 relative transition-all duration-500'>
                    <div className='absolute top-2 right-2 md:top-1 md:right-1 bg-dark h-full w-full -z-10 group-hover:-translate-x-[6px] group-hover:translate-y-[6px] transition-all duration-200'></div>
                    <div className='flex flex-col'>
                        <span className='fontmerienda font-bold text-base md:text-lg mb-1'>{name}</span>
